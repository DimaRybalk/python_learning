...
def converter(amount,source,target):
    for currency in currency_list:
        if currency["base"] == source:
            print(f"Ви перевели {amount} {source} в {currency["rates"][target]*amount} {target}")
        if source == target:
                print("Ви обрали однакові валюти 🙂")
...
target_currency = currency_validator("Введіть назву валюти в яку хочете перевести(USD/EUR/JPY/UAH): ")
